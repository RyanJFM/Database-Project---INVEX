...
function CaseManagement({ user }) {
  const handleDeleteCase = (caseId) => {
    if (window.confirm(`Are you sure you want to delete Forensic Case #FC-${caseId} and all its associated exam logs, evidence, files, and court reports? This action cannot be undone.`)) {
      axios.delete(`/api/cases/${caseId}`)
        .then(() => {
          alert('Forensic case and all associated records deleted successfully.');
          fetchCasesAndPatients();
        })
        .catch((err) => {
          console.error('Error deleting case:', err);
          alert(err.response?.data?.error || err.message || 'Failed to delete case');
        });
    }
  };
                    ...
                    <tr key={c.case_id}>
                      ...
                      <td style={{ textAlign: 'right' }}>
                        <div className="action-btn-group">
                          ...
                          <button className="action-btn action-btn-danger" onClick={() => handleDeleteCase(c.case_id)}>
                            <TrashIcon /> Delete
                          </button>
                        </div>
                      </td>
